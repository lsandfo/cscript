...
// Save last executed script as compiled executable
int save_data()
{
	char *buffer = NULL;
	unsigned long int file_size = 0;
	FILE *elf = fopen("/dev/shm/cscript.elf", "rb");
	FILE *dest = fopen("executable_cscript.elf", "wb");
	if (elf == NULL || dest == NULL)
	{
		perror("Save Executable");
		return 1;
	}
	...
	while ((fread(buffer, 1, sizeof(buffer), elf)) != 0){
